...
export default function EditarProdutos() {
  ...
  //recuperar o produto da lista de produtos do localStorage
  const listaDeProdutosString = localStorage.getItem("lista") || "[]";
  const lista = JSON.parse(listaDeProdutosString);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (produto) {
      const idProduto = lista.findIndex(
        (p: TipoProduto) => p.id === Number(id)
      );
